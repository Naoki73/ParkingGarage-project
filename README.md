...
Eric wrote the 'template' for the code (the first commit), found specifically to handle the input of the payment (finding out about pay=None in line 22), and found how to handle someone typing 0 payment into it to make it prompt until an amount above 0 is given (lines 47 and 48)

Russel helped provide many examples of other parking garages online, though many were far bigger and more complex than this, still were useful to take what we could from them, found how to handle finding out whether a dictionary is empty or not as opposed to just making it error from it (self.currentTicket.get , the .get part of it, line 39), and what we input to get it to work (lines 57-60)

Most of it besides these specific things were self-explanatory enough simply from the basic template Eric made in <7 minutes beforehand just from copying a past assignment, as well as just testing and fixing super easy mistakes from error messages, so it feels useless pointing out who did every single individual line